...
function getScheduleTimeString(schedule) {
    return `星期${dayMap.get(schedule.ssDayOfWeek)} 第 ${schedule.ssStartTime < 10 ? `0${schedule.ssStartTime}` : schedule.ssStartTime} ~ ${schedule.ssEndTime < 10 ? `0${schedule.ssEndTime}` : schedule.ssEndTime} 节`
}
